...
const InterConsultasDashboard = () => {
  const [interconsultasData, setInterconsultasData] = useState<any[]>([]);
  ...
  const especialidades = [...new Set(interconsultasData.map(item => item.diagnostico_estandarizado.especialidad_derivada))];
  const codigosCIE10 = [...new Set(interconsultasData.map(item => ({
    codigo: item.diagnostico_estandarizado.codigo_cie10,
    descripcion: item.diagnostico_estandarizado.descripcion
  ...
  const estadisticasEspecialidad = useMemo(() => {
    ...
    return Object.entries(conteos).map(([especialidad, cantidad]) => ({
      especialidad,
      cantidad
  ...
  const datosFiltrados = useMemo(() => {
    return interconsultasData.filter(item => {
      const cumpleEspecialidad = !filtroEspecialidad || item.diagnostico_estandarizado.especialidad_derivada === filtroEspecialidad;
      const cumpleCIE10 = !filtroCIE10 || item.diagnostico_estandarizado.codigo_cie10 === filtroCIE10;
      return cumpleEspecialidad && cumpleCIE10;
    });
  }, [filtroEspecialidad, filtroCIE10]);
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6 mb-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {estadisticasEspecialidad.map(({ especialidad, cantidad }) => {
            const totalPorcentaje = ((cantidad / interconsultasData.length) * 100).toFixed(1);
